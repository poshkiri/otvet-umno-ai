...
export function createBot(
  ...
  bot.command("paysupport", async (ctx) => {
    await ctx.reply(
      "Поддержка Пойми AI\n\nНаписать может любой пользователь, даже без подписки.\n\nЕсли пакет не начислился, нужен возврат или появилась ошибка, отправь сюда ID платежа и коротко опиши проблему. ID находится в разделе «Мои покупки».",
